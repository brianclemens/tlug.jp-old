---
categories:
- Linux_Help:QND-Guides|SSMTP
date: '2026-02-05'
draft: false
title: Linux Help - QND-Guides - Ssmtp
---
...
First follow the instructions in the QND software guide for information on getting and installing ssmtp. If it's not included with your distribution then you should be able to download it from [ibiblio](ftp://ftp.ibiblio.org/pub/Linux/system/mail/mta/).
...
<pre>
1. 1. /etc/ssmtp.conf -- a config file for sSMTP sendmail.
1. 1. The person who gets all mail for userids < 1000
CHANGE THIS LINE

root=postmaster

1. The place where the mail goes. The actual machine name is required
1. no MX records are consulted. Commonly mailhosts are named mail.domain.com
1. The example will fit if you are in domain.com and your mailhub is so named.
...
1. Example for SMTP port number 2525
1. mailhub=mail.your.domain:2525
1. Example for SMTP port number 25 (Standard/RFC)
1. mailhub=mail.your.domain        
1. Example for SSL encrypted connection
1. mailhub=mail.your.domain:465
...
1. Set this to never rewrite the "From:" line (unless not given) and to
1. use that address in the "from line" of the envelope.
...
1. Use SSL/TLS to send secure messages to server.
#UseTLS=YES

1. Use SSL/TLS certificate to authenticate against smtp host.
#UseTLSCert=YES

1. Use this RSA certificate.
