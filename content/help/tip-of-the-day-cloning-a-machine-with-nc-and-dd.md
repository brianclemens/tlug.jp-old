...
1. <pre>dd if=/dev/zero of=${HDD_DEV}</pre>
1. <pre>nc -l -p ${TARGET_PORT} | dd of=${HDD_DEV}</pre>
1. On the source machine:
1. <pre>HDD_DEV=/dev/hda</pre> Replace <code>/dev/hda</code> with the appropriate device name.
1. <pre>TARGET_IP=192.168.1.254</pre> Replace 192.168.1.254 with the correct IP address if you did not follow my advice because you know better. ;)
1. <pre>TARGET_PORT=2727</pre>
1. <pre>dd if=${HDD_DEV} | nc ${TARGET_IP} ${TARGET_PORT}</pre>
1. Wait for a while
